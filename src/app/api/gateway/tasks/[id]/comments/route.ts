...
// Module-level write lock — serializes concurrent POST requests to avoid read-modify-write races.
...
export async function POST(
  ...
  let savedComment!: StoredComment;
  const newLock = writeLock.then(async () => {
    const store = await loadStore();
